...
@section('content')
  <div class="d-flex">
    @if (Auth::user()->role <= 20 && (Request::is('validasi/4') || Request::is('validasi/3')))
    ...
    <div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true">
      ...
      <form method="get" class="px-7 py-5" action="{{ route('validasi.index', $status) }}">
        ...
        <div class="form-floating mb-10">
          <input value="{{ $cari ?? null }}" for="floatingInput" name="cari" type="text" class="form-control form-control-solid">
    ...
    <!--end::Menu 1-->
    <p class="mt-4 ms-auto">Total Peserta : {{ $count }}</p>
  </div>
              ...
              @if ($mahasiswa->status > 1)
                ...
                <div data-kt-menu="true"
                  ...
                  @if (Auth::user()->role == 10)
                    <div class="menu-item px-3">
                      <form action="{{ url('mahasiswa.delete', $mahasiswa->id) }}" method="post">@csrf
                        <a onclick="if(confirm('Yakin mau dihapus?')) {event.preventDefault(); this.closest('form').submit();} else {alert('Penghapusan dibatalkan')}"
                          class="menu-link px-3" data-kt-users-table-filter="delete_row">Hapus</a>
                      </form>
                    </div>
                  @endif
                  <!--end::Menu item-->
              ...
              <div class="modal-content">
                ...
                <form action="{{ route('validasi.update', $mahasiswa->id) }}" method="post">@csrf
                  ...
                  <div class="modal-footer">
                    ...
                    <button type="submit"
                      onClick="this.form.submit(); this.disabled=true; this.innerText='Mohon Tunggu…'; "
